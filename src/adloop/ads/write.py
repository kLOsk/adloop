...
def draft_ad_group(
    ...
    *,
    ...
    """Draft an ad group creation for an existing campaign."""
    from adloop.safety.guards import SafetyViolation, check_blocked_operation
    from adloop.safety.preview import ChangePlan, store_plan
    ...
    if max_cpc:
        uses_manual_cpc = _campaign_uses_manual_cpc(config, customer_id, campaign_id)
        if uses_manual_cpc is False:
            errors.append("max_cpc requires a MANUAL_CPC campaign")
        elif uses_manual_cpc is None:
            errors.append(
                f"Unable to verify bidding strategy for campaign_id '{campaign_id}'"
            )
...
def _validate_campaign(
    ...
    *,
    ...
    search_partners_enabled: bool,
    display_network_enabled: bool,
    max_cpc: float,
) -> tuple[list[str], list[str]]:
...
def _apply_create_ad_group(client: object, cid: str, changes: dict) -> dict:
    """Create a paused SEARCH_STANDARD ad group in an existing campaign."""

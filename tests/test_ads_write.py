...
def test_draft_ad_group_returns_preview_for_manual_cpc(config, monkeypatch):
    monkeypatch.setattr(write, "_campaign_uses_manual_cpc", lambda *_args: True)

    result = write.draft_ad_group(
        config,
    ...
    assert result["changes"]["max_cpc"] == 2.5


def test_draft_ad_group_rejects_max_cpc_without_manual_cpc(config, monkeypatch):
    monkeypatch.setattr(write, "_campaign_uses_manual_cpc", lambda *_args: False)
    ...
    assert "MANUAL_CPC" in result["details"][0]
...
def test_apply_create_ad_group_sets_manual_cpc_fields():
    ad_group_service = _FakeAdGroupService()
    client = _FakeClient(
        {
            "AdGroupService": ad_group_service,
            "CampaignService": _FakePathService("campaigns"),
        }
    )

    result = write._apply_create_ad_group(
        client,
        "1234567890",
        {"campaign_id": "1001", "ad_group_name": "Brand Terms", "max_cpc": 2.5},
    )

    operation = ad_group_service.operations[0]
    ad_group = operation.create
    assert ad_group.status == client.enums.AdGroupStatusEnum.PAUSED
    assert ad_group.type_ == client.enums.AdGroupTypeEnum.SEARCH_STANDARD
    assert ad_group.cpc_bid_micros == 2_500_000
    assert result["resource_name"].endswith("/adGroups/1")

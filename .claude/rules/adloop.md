...
| `get_reddit_ad_groups` | Budgets, bids, pixel, targeting per ad group | `ad_account_id`, `campaign_id` (optional) |
...
| `search_reddit_targeting` | Look up ids/names for ad-group targeting | `kind` (communities/interests/geolocations/languages/keywords), `query`, `country` |
...
| `update_reddit_campaign` | Name, schedule, `spend_cap`; budget/bid only on CBO campaigns | `daily_budget` or `lifetime_budget` (+ `end_time`) checked against `max_daily_budget`; preview shows old → new |
| `update_reddit_ad_group` | Budget, bid, schedule, targeting | `daily_budget` vs cap, `bid_value` vs `max_bid_increase_pct`; targeting lists REPLACE the keys you pass (pass full lists), other keys stay |
...
| `draft_reddit_ad_group` | Create an ad group (PAUSED) | `campaign_id`, `ad_group_name`, `conversion_pixel_id` (REQUIRED by Reddit; from `get_reddit_pixels`), at least one targeting list (geolocations/communities/interests/keywords), and for non-CBO campaigns `daily_budget` (or `lifetime_budget` + `end_time`), `bid_strategy` (BIDLESS, MANUAL_BIDDING, MAXIMIZE_VOLUME, TARGET_CPX), `bid_type` (CPC, CPM, CPV…); `optimization_goal` is the pixel event (PURCHASE, SIGN_UP, LEAD, PAGE_VISIT, …) |
| `draft_reddit_ad` | Create a post on the profile, then the ad (PAUSED) | `ad_group_id`, `profile_id` (from `list_reddit_funding_instruments`), `headline`, `click_url` (verified reachable), `post_type` TEXT or IMAGE (+ public `image_url`), `call_to_action` (Reddit's fixed labels: Learn More, Sign Up, Shop Now, Download, …), `allow_comments` |
...
3. **Targeting.** Ask which countries (and languages) to target if not stated; find ids/names with `search_reddit_targeting` (`geolocations` with `country`, `communities` by query, `interests`, `keywords`). Never draft an untargeted ad group; warn when a geo-targeted ad group has no language list
